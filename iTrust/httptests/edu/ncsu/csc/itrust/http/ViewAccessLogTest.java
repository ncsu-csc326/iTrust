...
public class ViewAccessLogTest extends iTrustHTTPTest {
	...
	/***
	 * Log in as a patient and attempt to enter a date in the View Access Log page in the format YYYY/MM/DD. Should get an error message
	 * specifying a bad date, and that the date is not properly formatted.
	 * Created by Daniel Rice for CSC326 Homework 2 Part 1.
	 */
	public void testViewAccessLogBadDateHandlingYearFirst() throws Exception {
		gen.clearAllTables();
		gen.standardData();
		
		WebConversation webCon = login("23", "pw");
		WebResponse wr = webCon.getCurrentPage();
		...
		WebForm dateForm = wr.getForms()[0];
		dateForm.setParameter("startDate", "2007/01/01");
		dateForm.setParameter("endDate", "1/21/2015");
		dateForm.submit();
		
		wr = webCon.getCurrentPage();
		assertEquals("iTrust - View My Access Log", wr.getTitle());
