...
public class RequestRecordsReleaseTest extends iTrustHTTPTest {
	 ...
	 */
	public void testMedicalRecordsRelease_Patient_NoSignature() 
			throws Exception {
		//Login as patient Caldwell Hudson
		WebConversation wc = login("102", "pw");	
		WebResponse wr = wc.getCurrentPage();
	 ...
	 * @throws Exception
	 */
	public void testMedicalRecordsRelease_Patient_NotAllFields() 
			throws Exception {
		//Login as patient Caldwell Hudson
		WebConversation wc = login("102", "pw");	
		WebResponse wr = wc.getCurrentPage();
		...
		//if we are still on the same page then we know that the the submission
		//failed and our test has bassed.
		assertEquals("iTrust - Records Release Request", wr.getTitle());
	...
	public void testPatientViewApprovedRequest() throws Exception {
		...
		assertTrue(wr.getText().contains("Email address: "
				+ "monica.brown@hartfordradiology.com"));
		assertTrue(wr.getText().contains("Hospital: "
				+ "Hartford Radiology Ltd."));
		assertTrue(wr.getText().contains("Hospital address: "
				+ "8941 Hargett Way, Hartford, CT 01243"));
	...
	public void testHCPApprovesRequest() throws Exception{
		...
		assertTrue(wr.getText().contains("Hospital address: 622 Center Wood "
				+ "Avenue, Savannah, GA 42991"));
	...
	public void testHCPDeniesRequest() throws Exception{
		...
		assertTrue(wr.getText().contains("Email address: "
				+ "mkgarrison@fairfaxchiro.com"));
		assertTrue(wr.getText().contains("Hospital: Fairfax Chiropractic"));
		assertTrue(wr.getText().contains("Hospital address: 72 Waywind "
				+ "Street, Hartford, CT 01241"));
	...
	public void testUAPDeniesRequest() throws Exception{
		...
		assertTrue(wr.getText().contains("Email address: "
				+ "c.dunbar@rexhospital.org"));
		assertTrue(wr.getText().contains("Hospital: Rex Hospital"));
		assertTrue(wr.getText().contains("Hospital address: 1829 Lake Boone "
				+ "Trail, Raleigh, NC 27612"));
		assertTrue(wr.getText().contains("Blood test requested from "
				+ "specialist"));
	...
	public void testUAPViewsApprovedRequest() throws Exception{
		...
		assertTrue(wr.getText().contains("Email address: "
				+ "hmcclain@easternhealth.com"));
		assertTrue(wr.getText().contains("Hospital: East Health Services"));
		assertTrue(wr.getText().contains("9002 Asheville Avenue, Cary, "
				+ "NC 27511"));
		assertTrue(wr.getText().contains("Referred for services"));
	}
	
	/**
	 * Views the dependents of a person
	 * @throws Exception
	 */
