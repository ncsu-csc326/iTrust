...
public class EditDemographicsTest extends iTrustHTTPTest {
	...
	public void testEditUpdatesImmediately() throws Exception {
		// Log into iTrust as Patient 1 (Random Person)
		WebConversation wc = login("1", "pw");
		WebResponse wr = wc.getCurrentPage();
		wr = wr.getLinkWith("My Demographics").click();
		
		// Change name on form and submit.
		wr.getForms()[0].setParameter("firstName", "Randomer");
		wr = wr.getForms()[0].submit(); // The form submits to itself.
		
		// Check that the name changed.
		assertEquals("Randomer", wr.getForms()[0].getParameterValue("firstName"));
	}
