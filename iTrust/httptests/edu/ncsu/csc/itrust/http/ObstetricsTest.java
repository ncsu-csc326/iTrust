...
public class ObstetricsTest extends iTrustHTTPTest {
	...
	public void testAddPatientEnterPrior() throws Exception {
		...
		// select specified patient
		WebForm patientForm = wr.getForms()[0];
		patientForm.getScriptableObject().setParameterValue("UID_PATIENTID","402");
		patientForm.getButtons()[1].click();
		// button click should trigger page redirect
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
		...
		// add the prior pregnancy information (TODO not here because form elements are JavaScript generated)
		//UPDATE: HttpUnit does do JavaScript onclick events. Just not complicated ones like this one. Stupid.
		//form.getButtonWithID("priorPregnancy").click();
		//form.getButtonWithID("priorPregnancy").getScriptableDelegate().doEvent("onclick");
		//System.out.println("Alert: " + wc.popNextAlert());
		//form = wr.getFormWithID("priorPregnancy0");
		//System.out.println("Formtext:" + form.getText());
		//form.setParameter("yearConception0", "2012");
		//form.getScriptableObject().setParameterValue("yearConception0", "2012");
	...
	public void testAddNonExistentPatient() throws Exception {
		...
		// select specified patient
		WebForm patientForm = wr.getForms()[0];
		patientForm.getScriptableObject().setParameterValue("UID_PATIENTID", "Somebody Else");
		wr = wc.getCurrentPage();
	...
	public void testAddPatientByID() throws Exception {
		...
		// select specified patient
		WebForm patientForm = wr.getForms()[0];
		patientForm.getScriptableObject().setParameterValue("UID_PATIENTID", "404");
		patientForm.getButtons()[1].click();
		// button click should trigger page redirect
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
	...
	public void testAddPatientFutureLMP() throws Exception {
		...
		// select specified patient
		WebForm patientForm = wr.getForms()[0];
		patientForm.getScriptableObject().setParameterValue("UID_PATIENTID", "405");
		patientForm.getButtons()[1].click();
		// button click should trigger page redirect
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
		...
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Initialize Obstetrics Record", wr.getTitle());
		assertTrue(wr.getText().contains("Last menstrual period cannot be after Date of visit"));
	}
	...
	public void testAddPatientChangeMind() throws Exception {
		...
		// select specified patient
		WebForm patientForm = wr.getForms()[0];
		patientForm.getScriptableObject().setParameterValue("UID_PATIENTID", "406");
		patientForm.getButtons()[1].click();
		// button click should trigger page redirect
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
	...
	public void testAddEditObstetricsOV() throws Exception {
		...
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
		assertTrue(wr.getText().contains("Obstetrics Office Visit successfully edited"));
	}
	...
	public void testAddObstetricsNonObHCP() throws Exception {
		...
		//enter somebody and submit to make sure you end up on the Edit Office Visit page
		WebForm patientForm = wr.getForms()[0];
	...
	public void testAddObstetricsOVPregnancyOver() throws Exception {
		...
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
		assertTrue(wr.getText().contains("The patient chosen is not a current obstetrics patient"));
	}
	...
	public void testAddObstetricsOVNotInitialized() throws Exception {
		...
		patientForm.getScriptableObject().setParameterValue("UID_PATIENTID", "21");
		patientForm.getButtons()[1].click();
		//button click should redirect to the obstetrics homepage, then throw an error and come back
		wr = wc.getCurrentPage();
		assertEquals("iTrust - Obstetrics", wr.getTitle());
		assertTrue(wr.getText().contains("The patient chosen is not a current obstetrics patient"));
	}
