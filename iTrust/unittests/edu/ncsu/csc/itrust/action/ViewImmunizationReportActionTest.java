...
public class ViewImmunizationReportActionTest extends TestCase {
	...
	public void testGetNeededImmunizations() throws Exception {
		...
		//Julia Roberts - Immunizations needed
		action = new ViewImmunizationReportAction(factory, 303L, 303L);
		needed = action.getNeededImmunizations(303L, 0);
		assertEquals(5, needed.size());
		assertEquals("90696", needed.get(0).getCptCode());
		assertEquals("90712", needed.get(1).getCptCode());
		assertEquals("90707", needed.get(2).getCptCode());
		assertEquals("90645", needed.get(3).getCptCode());
		assertEquals("90396", needed.get(4).getCptCode());
