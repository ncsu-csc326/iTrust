...
public class RecordsReleaseServletTest extends TestCase{
	...
	protected void setUp() throws Exception {
		...
		//request.getSession().getAttribute("loggedInName");
	...
	public void testRecordsReleasePostFail() throws Exception {
		...
		//when(request.getAttribute("failure")).thenReturn("This form has not been validated correctly. The following field are not properly filled in: [Doctor's phone number: xxx-xxx-xxxx]");
		//System.out.println(request.getAttribute("failure"));
		when(request.getRequestDispatcher("requestRecordsRelease.jsp")).thenReturn(rdisp);

		servlet.doPost(request, response);
		
		//assertEquals("This form has not been validated correctly. The following field are not properly filled in: [Doctor's phone number: xxx-xxx-xxxx]", request.getAttribute("failure"));		
	}
