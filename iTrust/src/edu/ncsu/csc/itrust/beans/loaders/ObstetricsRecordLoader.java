...
public class ObstetricsRecordLoader implements BeanLoader<ObstetricsRecordBean> {
	...
	public PreparedStatement loadParameters(PreparedStatement ps, ObstetricsRecordBean p) throws SQLException {
	    ...
	    //ps.setLong(i++,  p.getOid());
	    if (p.getLmp() != null)
	    	ps.setDate(i++, new java.sql.Date(p.getLmp().getTime())); //(Date) p.getLmp());
	    else
	    	ps.setDate(i++, new java.sql.Date(0));
	    if (p.getEdd() != null)
	    	ps.setDate(i++, new java.sql.Date(p.getEdd().getTime())); //(Date) p.getEdd());
	    else
	    	ps.setDate(i++, new java.sql.Date(0));
		ps.setString(i++, p.getWeeksPregnant());
		ps.setDate(i++, new java.sql.Date(p.getDateVisit().getTime())); //(Date) p.getDateVisit());
