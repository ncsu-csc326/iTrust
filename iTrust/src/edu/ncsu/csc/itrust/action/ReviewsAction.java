...
public class ReviewsAction {
   ...
   * @param mid reviewer ID (ie Patient)
   * @param dateOfReview java.util.Date obj of date reviewed
   * @param pid HCP under review ID 
   * @param rating overall int rating 
   * @param descriptiveReview String description of the Review
   * @throws DBException
   
  public void addReview(long mid, Date dateOfReview, long pid, int rating, String descriptiveReview) throws DBException{
	  ReviewsBean revb = new ReviewsBean();
	  if(dao.isRateable(mid, pid)){
		revb.setMID(mid);
	    revb.setPID(pid);
	    revb.setDescriptiveReview(descriptiveReview);
	    revb.setDateOfReview(dateOfReview);
        dao.addReview(revb);  
	  }
	  //TODO, we may need to add something here logically if the user 
	  //is trying to review a Dr. they don't already have 
