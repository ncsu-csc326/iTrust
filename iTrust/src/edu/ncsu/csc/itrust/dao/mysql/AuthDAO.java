...
public class AuthDAO {
	...
	public void recordLoginFailure(final String ipAddr) throws DBException {
		...
		try {
			...
			//pstmt.setInt(2, failures);
			//pstmt.setDate(3, Calendar.getInstance().getTime());
...
//		return HexUtils.toHexString(buf); //Switch to this statement if upgrade to using Apache Tomcat v7.0 or greater libraries
		return HexUtils.convert(buf); //Have to use Apache Tomcat v6.0 libraries
	}
}
