...
public class RemoteMonitoringDAO {
	...
	public List<RemoteMonitoringDataBean> getPatientsData(final long loggedInMID) throws DBException {
		...
		try {
			...
			pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringdata WHERE timelogged >= CURRENT_DATE ORDER BY PatientID, timeLogged DESC");
			final ResultSet dataRS = pstmt.executeQuery();
