...
public class DrugInteractionTest extends iTrustSeleniumTest {
	...
	public void testAddNewOverrideReason() throws Exception {
		...
		assertTrue(driver.findElement(By.cssSelector("BODY")).getText()
				.matches("^[\\s\\S]*Success: 22222 - Interaction not applicable to this patient added[\\s\\S]*$"));
		assertLogged(TransactionType.OVERRIDE_CODE_ADD, 9000000001L, 0L, "");
	}

	/*
	 * Authenticate admin 90000000001 Choose "Edit ND Codes" Choose
	 * "Tetracycline" Choose "Isotretinoin" interaction Click delete
	 */
	@Test
	public void testDeleteDrugInteraction() throws Exception {
		...
		// Warning: assertTextPresent may require manual changes
		// assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Interaction
		// deleted successfully[\\s\\S]*$"));
	}
	...
	/*
	 * Authenticate admin 90000000001 Choose "Edit ND Codes" Choose
	 * "Edit Interactions" Choose "Adefovir" as one drug Choose "Aspirin" as the
	 * other drug Enter
	...
	public void testRecordDrugInteraction() throws Exception {
		...
		assertEquals("iTrust - Maintain ND Codes", driver.getTitle());
		// new
		// Select(driver.findElement(By.name("drug1"))).selectByVisibleText("61958-0501
		// Adefovir");
		// new
		// Select(driver.findElement(By.name("drug2"))).selectByVisibleText("08109-6
		// Aspirin");
		driver.findElement(By.id("description")).clear();
		...
		// Warning: assertTextPresent may require manual changes
		// assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Interaction
		// recorded successfully[\\s\\S]*$"));
	}

	/*
	 * Authenticate admin 90000000001 Choose "Edit ND Codes" Choose
	 * "Edit Interactions" Choose "Adefovir" as both drug1 and drug2 Enter
	 * "Mixing this drug with itself will cause the person taking it to implode."
	 * Click submit
	 */
	@Test
	public void testRecordDrugInteractionSameDrugs() throws Exception {
		...
		assertEquals("iTrust - Maintain ND Codes", driver.getTitle());
		// ((Select)
		// driver.findElement(By.name("drug1"))).selectByVisibleText("61958-0501
		// Adefovir");
		// ((Select)
		// driver.findElement(By.name("drug2"))).selectByVisibleText("61958-0501
		// Adefovir");
		driver.findElement(By.id("description")).clear();
		driver.findElement(By.id("description"))
				.sendKeys("Mixing this drug with itself will cause the person taking it to implode.");
		driver.findElement(By.name("add")).click();
		// Warning: assertTextPresent may require manual changes
		// assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Interactions
		// can only be recorded between two different drugs [\\s\\S]*$"));
